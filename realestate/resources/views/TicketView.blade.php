...
<html lang="en">
   ...
   <div class="mb-3">
      ...
      <input type="text" id="Priority" value="{{ $ticket->priority }}" class="form-control mb-2" readonly>
            </div>
         <div class="mb-3">
      <label for="description"><strong>Description:</strong></label>
       <textarea id="desc" name="description" rows="4" class="form-control " readonly>{{ $ticket->description }}</textarea>
     </div>
         <h3>Comments:</h3>
    ...
    <ul class="list-group">
        @foreach ($ticket->comments as $comment)
            <li class="list-group-item">
                <strong>{{ $comment->employee->first_name ?? 'Unknown' }}:</strong>
                {{ $comment->content }}
                <br>
...
@endif
     ...
     <button class="edit-btn" onclick="window.location.href='{{ url('/Assign/' . $ticket->id) }}'"> Assign</button>
         </div>
           </form>
            </div>
             </div>
